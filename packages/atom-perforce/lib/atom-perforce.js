'use strict';
var isInClient = require('./is-in-client.js');
...
atomPerforce.exports = {
    ...
    setupEnvironment: function setupEnvironment() {
        var pathElements,
            ...
            // path.env may not function case-insensitively in Windows, in which case it may be 'Path'
        ...
        // TODO: remove this side-effect which mutates the global environment's PATH
        if(envPath && defaultPath) {
            pathElements = envPath.split(path.delimiter);
            if(pathElements.indexOf(defaultPath) === -1) {
                pathElements.unshift(defaultPath);
                envPath = pathElements.join(path.delimiter);
            }
        }
        ...
        if(editor && editor.getPath && editor.getPath()) {
            ...
            .then(function(p4Info) {
                if(isInClient(p4Info)) {
                    ...
                    atom.notifications.addInfo(openedBufferFilePath + ' is outside any known perforce workspace');
                }
            })
            .catch(function(err) {
                console.err(err);
            ...
            directories.forEach(function(dir) {
                ...
                .catch(function(err) {
                    ...
                    if(err.message && (/file\(s\) up-to-date/i).test(err.message)) {
                        ...
                        successDirectories.push(dir);
                        // TODO: may have problem if checkResolved throws. synDeferred.reject is not called.
                        return checkResolved(dir)
                        .then(function() {
                            syncDeferred.resolve();
                        });
        ...
        if(projectRoots && projectRoots.length) {
            projectRoots.forEach(function(projectRoot) {
                ...
                .then(function(p4InfoResult) {
                    p4Info = p4InfoResult;
                    if (isInClient(p4Info, projectRoot.path)) {
